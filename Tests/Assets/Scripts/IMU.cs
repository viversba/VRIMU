...
//	public float a;
	...
	public Text text;
	public GameObject arm,sowrd,rightHand;
	...
	private Vector3 offsetFromHardToSowrd;
	...
	void Awake () {
		...
		offsetFromHardToSowrd = rightHand.transform.position -  sowrd.transform.position;
	...
	void VerifyHealth(){
		...
		if (health <= 0) {
			...
			Destroy (gameObject);
			levelManager.LoadScene ("HighRateTerminal");
		}
	}

	void UpdatePositionAndRotation(){

		if (device.device.IsReading) {
			
			float gx, gy, gz, ax, ay, az, mx, my, mz;
			gx = Device.gyroscope.x;	
			gy = Device.gyroscope.y;
			gz = Device.gyroscope.z;
			ax = Device.accelerometer.x;	
			ay = Device.accelerometer.y;
			az = Device.accelerometer.z;
			mx = Device.magnetometer.x;
			my = Device.magnetometer.y;
			mz = Device.magnetometer.z;
			...
			Quaternion rotation = new Quaternion (quaternion [1], quaternion [0], quaternion [2], quaternion [3]);

			Quaternion offsetRotation = Quaternion.Inverse (initialRotation) * rotation;

			arm.transform.rotation = offsetRotation;
		} else {
		

		}

//		sowrd.transform.posion = arm.trtion = rightHand.transform.position + offsetFromHardToSowrd;
//		sowrd.transform.rotatiansform.rotation;
	}
